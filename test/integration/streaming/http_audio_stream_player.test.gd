...
func after_each():
	...
	await wait_for_signal(player, "open_completed", 0.1)
...
func test_can_play_file_shorter_than_one_second():
	...
	player.play()
	await wait_for_signal(player, "finished", 5)
...
func test_emits_started_signal_when_playback_started():
	...
	await wait_for_signal(player, "started", 1)
...
func test_play_from_near_the_end():
	...
	await wait_for_signal(player, "fully_finished", 4)
...
func test_playback_position_after_playing_for_a_few_seconds():
	...
	await wait_for_signal(player, "started", 1)
	await wait_seconds(3.375, "Play for a few seconds")
...
func test_not_buffering_when_fully_finished():
	...
	await wait_for_signal(player, "stopped", 1)
	assert_signal_emitted(player, "fully_finished")
